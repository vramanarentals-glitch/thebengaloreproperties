(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const l of document.querySelectorAll('link[rel="modulepreload"]'))r(l);new MutationObserver(l=>{for(const s of l)if(s.type==="childList")for(const t of s.addedNodes)t.tagName==="LINK"&&t.rel==="modulepreload"&&r(t)}).observe(document,{childList:!0,subtree:!0});function i(l){const s={};return l.integrity&&(s.integrity=l.integrity),l.referrerPolicy&&(s.referrerPolicy=l.referrerPolicy),l.crossOrigin==="use-credentials"?s.credentials="include":l.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function r(l){if(l.ep)return;l.ep=!0;const s=i(l);fetch(l.href,s)}})();const Q=["Anjanapura","Arekere","Ashok Nagar","Banashankari","Banaswadi","Bannerghatta Road","Basavanagudi","Basaveshwaranagar","Begur Road","Bellandur","Bommanahalli","Brigade Road","Brookefield","BTM Layout","Budigere Cross","Carmelaram","Chandra Layout","Chikkajala","Commercial Street","Cubbonpet","CV Raman Nagar","Devanahalli","Doddaballapur Road","Domlur","Electronic City Phase 1","Electronic City Phase 2","Giri Nagar","Goraguntepalya","Gottigere","HAL Airport Road","Haralur Road","Hebbal","Hennur Road","Hoodi","Horamavu","HRBR Layout","HSR Layout","Indiranagar","ITPL (Whitefield)","Jakkur","Jayanagar","JP Nagar","Kadugodi","Kaggadasapura","Kalyan Nagar","Kammanahalli","Kanakapura Road","Kasavanahalli","Kengeri","Koramangala","KR Puram (Krishnarajapuram)","Kudlu Gate","Kumara Park","Kumaraswamy Layout","Kundalahalli","Lavelle Road","Magadi Road","Mahadevapura","Mahalakshmi Layout","Malleshwaram","Manyata Tech Park (Nagavara)","Marathahalli","Mathikere","MG Road","Murugeshpalaya","Mysore Road","Nagarbhavi","Nagavara","Nandini Layout","Old Airport Road","Padmanabhanagar","Panathur","Peenya","Rajajinagar","Ramamurthy Nagar","Residency Road","Richmond Town","RT Nagar","Sadashivanagar","Sahakarnagar","Sanjay Nagar","Sarjapur Road","Seshadripuram","Shanthi Nagar","Shivajinagar","Singasandra","Thanisandra","Ulsoor (Halasuru)","Uttarahalli","Varthur","Vasanth Nagar","Vidyaranyapura","Vijayanagar","Whitefield","Wilson Garden","Yelahanka","Yelachenahalli","Yeshwanthpur"],T="/api";function H(){const a=localStorage.getItem("tbp_admin_token")||"",e="tbp_neon_super_admin_secret_key_2026_x89a";return{"Content-Type":"application/json","x-admin-token":a,"x-admin-key":e,Authorization:a?`Bearer ${a}`:`Bearer ${e}`}}const M={async getHealth(){try{const a=await fetch(`${T}/health`);if(!a.ok)throw new Error(`HTTP error! status: ${a.status}`);return await a.json()}catch(a){return console.warn("Backend API not responding, running with local cache:",a),null}},async verifyAdminToken(){try{const a=await fetch(`${T}/admin/verify-token`,{headers:H()});if(!a.ok)return!1;const e=await a.json();return!!(e&&e.valid)}catch{return!1}},async uploadImage(a,e=null,i="uploaded-property-photo.jpg",r="image/jpeg"){try{const l=await fetch(`${T}/upload-image`,{method:"POST",headers:H(),body:JSON.stringify({imageData:a,propertyId:e,fileName:i,mimeType:r})});if(!l.ok)throw new Error(`Image upload failed with status ${l.status}`);return await l.json()}catch(l){return console.error("Failed to upload image to Neon DB:",l),{success:!1,url:a,dataUrl:a}}},async getProperties(){try{const a=await fetch(`${T}/properties`);if(!a.ok)throw new Error(`Failed to fetch properties: ${a.status}`);return await a.json()}catch(a){return console.error("Failed to fetch properties from DB:",a),null}},async createProperty(a){try{const e=await fetch(`${T}/properties`,{method:"POST",headers:H(),body:JSON.stringify(a)});if(!e.ok){const i=await e.text();throw new Error(`Failed to create property in DB (${e.status}): ${i}`)}return await e.json()}catch(e){return console.error("Failed to create property in DB:",e),null}},async updateProperty(a,e){try{const i=await fetch(`${T}/properties/${a}`,{method:"PUT",headers:H(),body:JSON.stringify(e)});if(!i.ok)throw new Error(`Failed to update property: ${i.status}`);return await i.json()}catch(i){return console.error("Failed to update property in DB:",i),null}},async togglePropertyFlag(a,e){try{const i=await fetch(`${T}/properties/${a}/toggle-flag`,{method:"PATCH",headers:H(),body:JSON.stringify({flagName:e})});if(!i.ok)throw new Error(`Failed to toggle flag: ${i.status}`);return await i.json()}catch(i){return console.error("Failed to toggle property flag in DB:",i),null}},async updatePropertyFloor(a,e){try{const i=await fetch(`${T}/properties/${a}/floor`,{method:"PATCH",headers:H(),body:JSON.stringify({floor:e})});if(!i.ok)throw new Error(`Failed to update floor: ${i.status}`);return await i.json()}catch(i){return console.error("Failed to update property floor in DB:",i),null}},async deleteProperty(a){try{const e=await fetch(`${T}/properties/${a}`,{method:"DELETE",headers:H()});if(!e.ok){const i=await e.json().catch(()=>({}));return console.error(`Failed to delete property: ${e.status}`,i),{success:!1,status:e.status,error:i.error||`HTTP error ${e.status}`}}return await e.json()}catch(e){return console.error("Failed to delete property in DB:",e),{success:!1,error:e.message}}},async resetProperties(){try{const a=await fetch(`${T}/properties/reset`,{method:"POST",headers:H()});if(!a.ok)throw new Error(`Failed to reset properties: ${a.status}`);return await a.json()}catch(a){return console.error("Failed to reset properties in DB:",a),null}},async getLeads(){try{const a=await fetch(`${T}/leads`,{headers:H()});if(!a.ok)throw new Error(`Failed to fetch leads: ${a.status}`);return await a.json()}catch(a){return console.error("Failed to fetch leads from DB:",a),null}},async createLead(a){try{const e=await fetch(`${T}/leads`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)});if(!e.ok)throw new Error(`Failed to create lead: ${e.status}`);return await e.json()}catch(e){return console.error("Failed to create lead in DB:",e),a}},async updateLead(a,e){try{const i=await fetch(`${T}/leads/${a}`,{method:"PATCH",headers:H(),body:JSON.stringify(e)});if(!i.ok)throw new Error(`Failed to update lead: ${i.status}`);return await i.json()}catch(i){return console.error("Failed to update lead in DB:",i),null}},async deleteLead(a){try{const e=await fetch(`${T}/leads/${a}`,{method:"DELETE",headers:H()});if(!e.ok)throw new Error(`Failed to delete lead: ${e.status}`);return await e.json()}catch(e){return console.error("Failed to delete lead in DB:",e),null}},async registerUser(a){try{return await(await fetch(`${T}/auth/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)})).json()}catch(e){return console.error("Registration failed:",e),{success:!1,message:"Connection error during registration"}}},async loginUser(a,e){try{return await(await fetch(`${T}/auth/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:a,password:e})})).json()}catch(i){return console.error("Login failed:",i),{success:!1,message:"Connection error during login"}}},async getContactInfo(){try{const a=await fetch(`${T}/contact`);if(!a.ok)throw new Error(`Failed to fetch contact info: ${a.status}`);return await a.json()}catch(a){return console.error("Failed to fetch contact info from DB:",a),null}},async updateContactInfo(a){try{const e=await fetch(`${T}/contact`,{method:"PUT",headers:H(),body:JSON.stringify(a)});if(!e.ok)throw new Error(`Failed to update contact info: ${e.status}`);return await e.json()}catch(e){return console.error("Failed to update contact info in DB:",e),null}}};class Z{constructor(){try{const p="v7_all_properties_removed";localStorage.getItem("tbp_sync_ver")!==p&&(localStorage.removeItem("tbp_properties"),localStorage.removeItem("tbp_deleted_ids"),localStorage.removeItem("tbp_leads"),localStorage.removeItem("tbp_favorites"),localStorage.setItem("tbp_sync_ver",p))}catch{}const e=localStorage.getItem("tbp_properties");this.allProperties=e?JSON.parse(e):[],this.filteredProperties=[...this.allProperties],this.filters={searchQuery:"",locality:"All",bhk:"All",maxPrice:15e4,minPrice:15e3,furnishing:"All",tenantType:"All",zeroBrokerageOnly:!1,verifiedOnly:!1,amenities:[]},this.sortBy="newest",this.viewMode="grid";const i=localStorage.getItem("tbp_favorites");this.favorites=i?JSON.parse(i):[];const r=localStorage.getItem("tbp_theme")||"light";this.theme=r;const l=localStorage.getItem("tbp_contact_info"),s={name:"The Bangalore Properties",proprietor:"V. RAMANA",role:"Proprietor",phone:"+91 80504 07710",phoneRaw:"+918050407710",whatsapp:"+918050407710",email:"ramuramana92@gmail.com",address:"Ground floor, Srinivas Residency, 2nd Main, KR Garden, Murugeshpalaya, Bangalore - 560017",locality:"Murugeshpalaya",slogan:"YOUR PROPERTY, OUR PRIORITY.",services:["RESIDENTIAL RENT","COMMERCIAL RENT","OFFICE SPACE RENT","GODOWN SPACE RENT","LONG TERM LEASE RENT"]};if(this.contactInfo=l?JSON.parse(l):s,typeof localStorage<"u"&&localStorage.getItem("tbp_admin_pw_version")!=="2026_ramana@123"){localStorage.removeItem("tbp_admin_auth"),localStorage.removeItem("tbp_admin_token");try{const p=JSON.parse(localStorage.getItem("tbp_user")||"null");p&&(p.isAdmin||p.email==="vramanarentals@gmail.com")&&localStorage.removeItem("tbp_user")}catch{}localStorage.setItem("tbp_admin_pw_version","2026_ramana@123")}const t=localStorage.getItem("tbp_user");this.currentUser=t?JSON.parse(t):null,this.currentUser&&this.currentUser.email==="vramanarentals@gmail.com"&&!this.currentUser.isAdmin&&(this.currentUser=null,localStorage.removeItem("tbp_user"));const f=localStorage.getItem("tbp_registered_users");this.registeredUsers=f?JSON.parse(f):[],this.authTab="email-pass",this.userAuthMode="signin",this.pendingEmail="",this.activeModal=null,this.activeProperty=null;const c=localStorage.getItem("tbp_admin_auth")==="true";this.isAdminLoggedIn=!!(this.currentUser&&this.currentUser.isAdmin&&c),this.adminTab="dashboard";const m=localStorage.getItem("tbp_leads");this.leads=m?JSON.parse(m):[],this.dbStatus="connecting",this.listeners=[],this.initFromDb(),setInterval(()=>this.initFromDb(),1e4),typeof window<"u"&&(window.addEventListener("focus",()=>this.initFromDb()),document.addEventListener("visibilitychange",()=>{document.visibilityState==="visible"&&this.initFromDb()}))}async initFromDb(){try{let e=!1;const i=await M.getHealth(),r=i&&i.status==="ok"?"connected":"offline";this.dbStatus!==r&&(this.dbStatus=r,e=!0),localStorage.getItem("tbp_admin_auth")==="true"&&!this.isAdminLoggedIn&&(this.isAdminLoggedIn=!0,e=!0);const l=await M.getProperties();if(l&&Array.isArray(l)){const f=JSON.parse(localStorage.getItem("tbp_deleted_ids")||"[]"),c=l.filter(m=>!f.includes(m.id));JSON.stringify(c)!==JSON.stringify(this.allProperties)&&(this.allProperties=c,this.applyFilters(),this.saveProperties(),e=!0)}const s=await M.getLeads();s&&Array.isArray(s)&&JSON.stringify(s)!==JSON.stringify(this.leads)&&(this.leads=s,localStorage.setItem("tbp_leads",JSON.stringify(this.leads)),e=!0);const t=await M.getContactInfo();t&&JSON.stringify(t)!==JSON.stringify(this.contactInfo)&&(this.contactInfo=t,localStorage.setItem("tbp_contact_info",JSON.stringify(this.contactInfo)),e=!0),e&&(this.applyFilters(),this.notify())}catch{this.dbStatus!=="offline"&&(this.dbStatus="offline",this.notify())}}handleGoogleCredential(e){try{const r=e.credential.split(".")[1].replace(/-/g,"+").replace(/_/g,"/"),l=decodeURIComponent(atob(r).split("").map(f=>"%"+("00"+f.charCodeAt(0).toString(16)).slice(-2)).join("")),s=JSON.parse(l),t={id:`usr-google-${s.sub}`,name:s.name||s.given_name||s.email.split("@")[0],email:s.email,provider:"Google Account",avatar:s.picture||"https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=150&q=80"};return this.currentUser=t,localStorage.setItem("tbp_user",JSON.stringify(t)),this.activeModal=null,this.notify(),t}catch(i){return console.error("Failed to parse Google OAuth credential:",i),this.loginWithGoogle()}}loginWithGoogle(e="user.google@gmail.com",i="Google User"){const r={id:`usr-google-${Date.now()}`,name:i,email:e,provider:"Google Account",avatar:"https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=150&q=80"};return this.currentUser=r,localStorage.setItem("tbp_user",JSON.stringify(r)),this.activeModal=null,this.notify(),r}logout(){this.currentUser=null,this.isAdminLoggedIn=!1,localStorage.removeItem("tbp_user"),localStorage.removeItem("tbp_admin_auth"),localStorage.removeItem("tbp_admin_token"),window.location.pathname==="/admin"&&history.replaceState(null,"","/"),this.notify()}subscribe(e){return this.listeners.push(e),()=>{this.listeners=this.listeners.filter(i=>i!==e)}}notify(){this.applyFilters(),this.listeners.forEach(e=>e(this))}setTheme(e){this.theme=e,localStorage.setItem("tbp_theme",e),document.documentElement.setAttribute("data-theme",e),this.notify()}toggleTheme(){const e=this.theme==="dark"?"light":"dark";this.setTheme(e)}toggleFavorite(e){this.favorites.includes(e)?this.favorites=this.favorites.filter(i=>i!==e):this.favorites.push(e),localStorage.setItem("tbp_favorites",JSON.stringify(this.favorites)),this.notify()}isFavorite(e){return this.favorites.includes(e)}updateFilter(e,i){this.filters[e]=i,this.notify()}toggleAmenity(e){const i=this.filters.amenities.indexOf(e);i>-1?this.filters.amenities.splice(i,1):this.filters.amenities.push(e),this.notify()}resetFilters(){this.filters={searchQuery:"",locality:"All",bhk:"All",maxPrice:15e4,minPrice:15e3,furnishing:"All",tenantType:"All",zeroBrokerageOnly:!1,verifiedOnly:!1,amenities:[]},this.sortBy="newest",this.notify()}setSortBy(e){this.sortBy=e,this.notify()}openModal(e,i=null){this.activeModal=e,this.activeProperty=i,e==="admin-portal"&&window.location.pathname!=="/admin"&&!window.location.hash.includes("admin")&&history.pushState(null,"","/admin"),this.notify()}closeModal(){this.activeModal==="admin-portal"&&window.location.pathname==="/admin"&&history.replaceState(null,"","/"),this.activeModal=null,this.activeProperty=null,this.notify()}async addProperty(e){const i={id:e.id||`prop-custom-${Date.now()}`,isVerified:!0,isFeatured:!0,zeroBrokerage:!0,images:["https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?auto=format&fit=crop&w=1200&q=80"],...e},r=JSON.parse(localStorage.getItem("tbp_deleted_ids")||"[]");r.includes(i.id)&&localStorage.setItem("tbp_deleted_ids",JSON.stringify(r.filter(s=>s!==i.id)));const l=this.allProperties.findIndex(s=>s.id===i.id);l!==-1?this.allProperties[l]=i:this.allProperties.unshift(i),this.saveProperties(),this.notify();try{const s=await M.createProperty(i);if(s&&s.id){const t=this.allProperties.findIndex(f=>f.id===i.id);t!==-1&&(this.allProperties[t]=s,this.saveProperties(),this.notify())}}catch(s){console.error("Failed to persist property to Neon DB:",s)}}saveProperties(){try{localStorage.setItem("tbp_properties",JSON.stringify(this.allProperties))}catch(e){console.error("Failed to save properties to localStorage:",e)}}async resetPropertiesToDefault(){try{const e=await M.resetProperties();e&&Array.isArray(e)?this.allProperties=e:this.allProperties=[]}catch{this.allProperties=[]}this.saveProperties(),this.notify()}applyFilters(){let e=[...this.allProperties];if(this.filters.searchQuery.trim()!==""){const i=this.filters.searchQuery.toLowerCase();e=e.filter(r=>r.title.toLowerCase().includes(i)||r.locality.toLowerCase().includes(i)||r.address&&r.address.toLowerCase().includes(i)||r.bhk&&r.bhk.toLowerCase().includes(i))}this.filters.locality!=="All"&&(e=e.filter(i=>i.locality===this.filters.locality)),this.filters.bhk!=="All"&&(e=e.filter(i=>i.bhkType===this.filters.bhk)),e=e.filter(i=>i.price<=this.filters.maxPrice),this.filters.furnishing!=="All"&&(e=e.filter(i=>i.furnishing===this.filters.furnishing)),this.filters.zeroBrokerageOnly&&(e=e.filter(i=>i.zeroBrokerage)),this.filters.verifiedOnly&&(e=e.filter(i=>i.isVerified)),this.filters.amenities.length>0&&(e=e.filter(i=>this.filters.amenities.every(r=>i.amenities&&i.amenities.includes(r)))),this.sortBy==="price-low"?e.sort((i,r)=>i.price-r.price):this.sortBy==="price-high"?e.sort((i,r)=>r.price-i.price):e.sort((i,r)=>r.id>i.id?1:-1),this.filteredProperties=e}async registerUser({name:e,email:i,password:r}){if(!e||!i||!r)return{success:!1,message:"Please fill in all required fields."};const l=await M.registerUser({name:e,email:i,password:r});return l&&l.success&&l.user?(this.currentUser=l.user,localStorage.setItem("tbp_user",JSON.stringify(l.user)),this.closeModal(),this.notify(),l):l||{success:!1,message:"Registration failed."}}async loginUser(e,i){if(!e||!i)return{success:!1,message:"Please enter both email and password."};const r=e.trim().toLowerCase(),l=i.trim();if(l.toLowerCase()==="ramana rentals")return{success:!1,message:"Access Denied: Invalid administrator credentials."};const s=await M.loginUser(r,l);if(s&&s.success&&s.user)return this.currentUser=s.user,this.isAdminLoggedIn=!!s.isAdmin,localStorage.setItem("tbp_user",JSON.stringify(s.user)),s.isAdmin?(localStorage.setItem("tbp_admin_auth","true"),localStorage.setItem("tbp_admin_pw_version","2026_ramana@123"),s.token&&localStorage.setItem("tbp_admin_token",s.token)):(localStorage.removeItem("tbp_admin_auth"),localStorage.removeItem("tbp_admin_token")),this.closeModal(),this.notify(),s;if((!s||!s.success)&&r==="vramanarentals@gmail.com"&&l==="ramana@123"){const t={id:"usr-admin",name:"V. RAMANA (Proprietor)",email:"vramanarentals@gmail.com",provider:"Admin Account",avatar:"https://api.dicebear.com/7.x/avataaars/svg?seed=VRamana",isAdmin:!0};return this.currentUser=t,this.isAdminLoggedIn=!0,localStorage.setItem("tbp_user",JSON.stringify(t)),localStorage.setItem("tbp_admin_auth","true"),localStorage.setItem("tbp_admin_pw_version","2026_ramana@123"),localStorage.setItem("tbp_admin_token","tbp_offline_admin_token_2026"),this.closeModal(),this.notify(),{success:!0,user:t,isAdmin:!0,token:"tbp_offline_admin_token_2026"}}return s||{success:!1,message:"Invalid credentials."}}isAdmin(){const e=typeof window<"u"&&localStorage.getItem("tbp_admin_auth")==="true";return!!((this.isAdminLoggedIn||e)&&this.currentUser&&this.currentUser.isAdmin)}async adminLogin(e,i){if(!e||!i||i.trim().toLowerCase()==="ramana rentals")return!1;const l=await this.loginUser(e,i);return!!(l&&l.success&&l.isAdmin)}adminLogout(){var e;this.isAdminLoggedIn=!1,localStorage.removeItem("tbp_admin_auth"),localStorage.removeItem("tbp_admin_token"),(e=this.currentUser)!=null&&e.isAdmin&&(this.currentUser=null,localStorage.removeItem("tbp_user")),this.closeModal(),window.location.pathname==="/admin"&&history.replaceState(null,"","/"),this.notify()}setAdminTab(e){this.adminTab=e,this.notify()}async deleteProperty(e){const i=JSON.parse(localStorage.getItem("tbp_deleted_ids")||"[]");i.includes(e)||(i.push(e),localStorage.setItem("tbp_deleted_ids",JSON.stringify(i))),this.allProperties=this.allProperties.filter(r=>r.id!==e),this.saveProperties(),this.notify();try{await M.deleteProperty(e)}catch(r){console.warn("DB delete error:",r)}return{success:!0}}async updateProperty(e,i){const r=this.allProperties.findIndex(l=>l.id===e);if(r!==-1){const l={...this.allProperties[r],...i};this.allProperties[r]=l,this.activeProperty&&this.activeProperty.id===e&&(this.activeProperty=l),this.applyFilters(),this.saveProperties(),this.notify();try{const s=await M.updateProperty(e,i);if(s&&s.id){const t={...this.allProperties[r],...s};this.allProperties[r]=t,this.activeProperty&&this.activeProperty.id===e&&(this.activeProperty=t),this.saveProperties(),this.notify()}}catch(s){console.warn("DB update error:",s)}return{success:!0}}return{success:!1}}async togglePropertyFlag(e,i){const r=this.allProperties.find(l=>l.id===e);r&&(r[i]=!r[i],this.saveProperties(),this.notify(),await M.togglePropertyFlag(e,i))}async updatePropertyFloor(e,i){const r=this.allProperties.find(l=>l.id===e);r&&(r.floor=i,this.saveProperties(),this.notify(),await M.updatePropertyFloor(e,i))}async updateLeadStatus(e,i){const r=this.leads.find(l=>l.id===e);r&&(r.status=i,localStorage.setItem("tbp_leads",JSON.stringify(this.leads)),this.notify(),await M.updateLead(e,{status:i}))}async deleteLead(e){this.leads=this.leads.filter(i=>i.id!==e),localStorage.setItem("tbp_leads",JSON.stringify(this.leads)),this.notify(),await M.deleteLead(e)}async addLead(e){const i={id:`lead-${Date.now()}`,date:new Date().toISOString().split("T")[0],status:"New",...e};this.leads.unshift(i),localStorage.setItem("tbp_leads",JSON.stringify(this.leads)),this.notify(),await M.createLead(i)}async updateContactInfo(e){this.contactInfo={...this.contactInfo,...e};try{localStorage.setItem("tbp_contact_info",JSON.stringify(this.contactInfo))}catch(i){console.error("Failed to save contact info to localStorage:",i)}this.notify(),await M.updateContactInfo(this.contactInfo)}}const o=new Z;function ee(){var r,l,s,t,f,c,m;const a=document.getElementById("header-root");if(!a)return;o.favorites.length;const e=o.theme==="dark",i=typeof o.isAdmin=="function"?o.isAdmin():!1;a.innerHTML=`
        ...
        <div class="header-actions">
          ...
          ${o.currentUser?`
            <div style="display: flex; align-items: center; gap: 0.35rem;">
              <button id="btn-user-profile" class="nav-btn" style="background: rgba(16, 185, 129, 0.15); border: 1px solid rgba(16, 185, 129, 0.4); color: #10b981; font-weight: 700;" title="Logged in as ${o.currentUser.name}">
  ...
  `,(r=document.getElementById("btn-header-upload-prop"))==null||r.addEventListener("click",()=>{i?(o.setAdminTab("add-property"),o.openModal("admin-portal")):(o.pendingAdminTab="add-property",o.openModal("admin-portal"))}),(l=document.getElementById("btn-header-auth"))==null||l.addEventListener("click",()=>{o.openModal("auth-signin")}),(s=document.getElementById("btn-header-admin-portal"))==null||s.addEventListener("click",()=>{o.openModal("admin-portal")}),(t=document.getElementById("btn-user-logout"))==null||t.addEventListener("click",()=>{o.logout()}),(f=document.getElementById("btn-book-call"))==null||f.addEventListener("click",()=>{o.openModal("book-call")}),(c=document.getElementById("btn-contact-card"))==null||c.addEventListener("click",()=>{o.openModal("contact-us")}),(m=document.getElementById("btn-theme-toggle"))==null||m.addEventListener("click",()=>{o.toggleTheme()})}function te(){var i,r,l,s;const a=document.getElementById("hero-root");if(!a)return;const e=o.contactInfo;a.innerHTML=`
  ...
  `,(i=document.getElementById("hero-search-form"))==null||i.addEventListener("submit",t=>{var c;t.preventDefault();const f=(c=document.getElementById("hero-place-search-input"))==null?void 0:c.value;f!==void 0&&o.updateFilter("searchQuery",f)}),(r=document.getElementById("hero-place-search-input"))==null||r.addEventListener("input",t=>{o.updateFilter("searchQuery",t.target.value)}),(l=document.getElementById("hero-btn-book-call"))==null||l.addEventListener("click",()=>{o.openModal("book-call")}),(s=document.getElementById("hero-btn-view-card"))==null||s.addEventListener("click",()=>{o.openModal("contact-us")})}function ie(){var r,l,s;const a=document.getElementById("filters-root");if(!a)return;const e=o.filters;a.innerHTML=`
  ...
  `,(r=document.getElementById("btn-reset-all"))==null||r.addEventListener("click",()=>{o.resetFilters()}),(l=document.getElementById("filters-search-form"))==null||l.addEventListener("submit",t=>{var c;t.preventDefault();const f=(c=document.getElementById("filter-search-input"))==null?void 0:c.value;f!==void 0&&o.updateFilter("searchQuery",f)});const i=document.getElementById("filter-search-input");i==null||i.addEventListener("input",t=>{o.updateFilter("searchQuery",t.target.value)}),a.querySelectorAll("[data-bhk]").forEach(t=>{t.addEventListener("click",()=>{o.updateFilter("bhk",t.dataset.bhk)})}),a.querySelectorAll("[data-furnish]").forEach(t=>{t.addEventListener("click",()=>{o.updateFilter("furnishing",t.dataset.furnish)})}),(s=document.getElementById("filter-price-slider"))==null||s.addEventListener("input",t=>{o.updateFilter("maxPrice",Number(t.target.value))})}function ae(){var r,l,s;const a=document.getElementById("quick-bar-root"),e=document.getElementById("property-grid-root");if(!e)return;const i=o.filteredProperties.length;if(a&&(a.innerHTML=`
      ...
      <div class="sort-container">
        ...
        <select id="sort-select" class="sort-select">
          <option value="newest" ${o.sortBy==="newest"?"selected":""}>Newest Added</option>
          <option value="price-low" ${o.sortBy==="price-low"?"selected":""}>Price: Low to High</option>
          <option value="price-high" ${o.sortBy==="price-high"?"selected":""}>Price: High to Low</option>
        </select>
      </div>
    `,(r=document.getElementById("sort-select"))==null||r.addEventListener("change",t=>{o.setSortBy(t.target.value)})),i===0){const t=o.contactInfo,f=o.filters.searchQuery.trim()||(o.filters.locality!=="All"?o.filters.locality:""),c=f||"your selected search criteria",m=f?`Hello V. Ramana, I searched for rental properties in "${f}" on The Bangalore Properties website, but no active listings were displayed. Please send me available options for "${f}".`:"Hello V. Ramana, I am looking for rental properties in Bangalore with my specific requirements. Please share available options.",p=`https://wa.me/${t.whatsapp.replace("+","")}?text=${encodeURIComponent(m)}`;e.innerHTML=`
    ...
    `,(l=document.getElementById("btn-empty-book-call"))==null||l.addEventListener("click",()=>{o.openModal("book-call")}),(s=document.getElementById("btn-empty-reset"))==null||s.addEventListener("click",()=>{o.resetFilters()});return}e.innerHTML=o.filteredProperties.map(t=>`
    ...
    `).join(""),e.querySelectorAll(".btn-view-details").forEach(t=>{t.addEventListener("click",()=>{const f=t.dataset.propId,c=o.allProperties.find(m=>m.id===f);c&&o.openModal("property-details",c)})}),e.querySelectorAll(".btn-book-call-card").forEach(t=>{t.addEventListener("click",()=>{const f=t.dataset.propId,c=o.allProperties.find(m=>m.id===f);o.openModal("book-call",c)})}),e.querySelectorAll(".btn-schedule-visit").forEach(t=>{t.addEventListener("click",()=>{const f=t.dataset.propId,c=o.allProperties.find(m=>m.id===f);c&&o.openModal("schedule-visit",c)})})}function $(a,e="fa-circle-check"){const i=document.getElementById("toast-container");if(!i)return;const r=document.createElement("div");r.className="toast",r.innerHTML=`
    <i class="fa-solid ${e}" style="color: var(--accent-emerald); font-size: 1.2rem;"></i>
    <span>${a}</span>
  `,i.appendChild(r),setTimeout(()=>{r.style.opacity="0",r.style.transform="translateX(50px)",r.style.transition="all 0.3s ease",setTimeout(()=>{r.remove()},300)},4500)}function oe(){var l,s,t,f,c,m,p,n,d,u,b,v,P,C,N,q,j,_;const a=document.getElementById("modal-root");if(!a)return;if(!o.activeModal){a.innerHTML="";return}const e=o.activeProperty,i=typeof o.isAdmin=="function"?o.isAdmin():!1;if(o.activeModal==="property-details"&&e){a.innerHTML=`
    ...
    `,(l=document.getElementById("btn-back-details"))==null||l.addEventListener("click",()=>{o.closeModal()});const g=document.getElementById("property-gallery-slider"),S=document.getElementById("gallery-active-index"),A=a.querySelectorAll(".gallery-thumb-btn");if(g){let w=!1,k=0,F=0;g.addEventListener("mousedown",h=>{w=!0,g.classList.add("is-dragging"),k=h.pageX-g.offsetLeft,F=g.scrollLeft}),window.addEventListener("mouseup",()=>{if(!w)return;w=!1,g.classList.remove("is-dragging");const h=g.clientWidth;if(h>0){const O=Math.round(g.scrollLeft/h);g.scrollTo({left:O*h,behavior:"smooth"})}}),g.addEventListener("mousemove",h=>{if(!w)return;h.preventDefault();const R=(h.pageX-g.offsetLeft-k)*1.5;g.scrollLeft=F-R});let E=0,y=0,x=0,B=0,z=!1;g.addEventListener("touchstart",h=>{!h.touches||h.touches.length===0||(E=h.touches[0].clientX,y=h.touches[0].clientY,x=Date.now(),B=0,z=!0)},{passive:!0}),g.addEventListener("touchmove",h=>{if(!z||!h.touches||h.touches.length===0)return;const O=h.touches[0].clientX,R=h.touches[0].clientY;B=O-E;const D=R-y;Math.abs(B)>Math.abs(D)&&Math.abs(B)>10&&h.cancelable&&h.preventDefault()},{passive:!1}),g.addEventListener("touchend",()=>{if(!z)return;z=!1;const h=g.clientWidth,R=Date.now()-x<300&&Math.abs(B)>20,D=Math.abs(B)>35;if(h>0&&(R||D)){const U=Math.round(g.scrollLeft/h);B<0&&U<e.images.length-1?g.scrollTo({left:(U+1)*h,behavior:"smooth"}):B>0&&U>0?g.scrollTo({left:(U-1)*h,behavior:"smooth"}):g.scrollTo({left:U*h,behavior:"smooth"})}},{passive:!0});let L;g.addEventListener("scroll",()=>{clearTimeout(L),L=setTimeout(()=>{const h=g.clientWidth;if(h<=0)return;const O=Math.round(g.scrollLeft/h);S&&(S.textContent=String(O+1)),A.forEach((R,D)=>{D===O?(R.classList.add("active"),R.scrollIntoView({behavior:"smooth",inline:"center",block:"nearest"})):R.classList.remove("active")})},50)},{passive:!0}),A.forEach(h=>{h.addEventListener("click",O=>{O.preventDefault();const R=Number(h.dataset.thumbIndex),D=g.clientWidth;g.scrollTo({left:R*D,behavior:"smooth"})})})}(s=document.getElementById("btn-modal-book-call"))==null||s.addEventListener("click",()=>{o.openModal("book-call",e)}),(t=document.getElementById("btn-modal-schedule-tour"))==null||t.addEventListener("click",()=>{o.openModal("schedule-visit",e)});const I=w=>{if(w==null||w.preventDefault(),!i){$("🔒 Only administrators can edit properties.");return}o.openModal("edit-property",e)};(f=document.getElementById("btn-edit-details"))==null||f.addEventListener("click",I),(c=document.getElementById("btn-badge-edit-details"))==null||c.addEventListener("click",I),(m=document.getElementById("btn-side-edit-details"))==null||m.addEventListener("click",I)}else if(o.activeModal==="schedule-visit"&&e){const g=new Date().toISOString().split("T")[0];a.innerHTML=`
    ...
    `;let S="10:00 AM";a.querySelectorAll("#time-slot-pills .pill-btn").forEach(A=>{A.addEventListener("click",()=>{a.querySelectorAll("#time-slot-pills .pill-btn").forEach(I=>I.classList.remove("active")),A.classList.add("active"),S=A.dataset.slot})}),(p=document.getElementById("form-schedule-visit"))==null||p.addEventListener("submit",A=>{A.preventDefault();const I=document.getElementById("visit-date").value,w=document.getElementById("visit-name").value,k=document.getElementById("visit-phone").value;o.addLead({tenantName:w,tenantPhone:k,propertyId:e.id,propertyTitle:e.title,locality:e.locality,date:I,status:"New",notes:`Property tour booked for ${I} at ${S}. Property: ${e.title} (${e.locality})`}),o.closeModal(),$(`🎉 Tour Confirmed for ${w}! Appointment scheduled for ${I} at ${S}. Confirmation SMS sent to ${k}.`)})}else if(o.activeModal==="contact-us"){const g=o.contactInfo;a.innerHTML=`
      <div class="modal-overlay" id="modal-backdrop">
...
Services: Rent, Lease, Sale, Office Space, Godown Space, etc.`).then(()=>{$("📋 Contact details copied to clipboard!")}).catch(()=>{$("📋 V. RAMANA (+91 80504 07710)")})})}else if(o.activeModal==="book-call"){const g=new Date().toISOString().split("T")[0],S=o.contactInfo;a.innerHTML=`
      <div class="modal-overlay" id="modal-backdrop">
    ...
    `,(d=document.getElementById("form-book-call"))==null||d.addEventListener("submit",A=>{A.preventDefault();const I=document.getElementById("book-call-name").value.trim(),w=document.getElementById("book-call-phone").value.trim(),k=document.getElementById("book-call-service").value,F=document.getElementById("book-call-locality").value.trim()||"Bangalore",E=document.getElementById("book-call-date").value,y=document.getElementById("book-call-slot").value,x=document.getElementById("book-call-notes").value.trim();o.addLead({tenantName:I,tenantPhone:w,propertyTitle:`Booked Call: ${k}`,locality:F,date:E,notes:`Call Scheduled for ${E} [${y}]. Service: ${k}. ${x?"Notes: "+x:""}`}),o.closeModal(),$(`📞 Call Request Booked! Proprietor ${S.proprietor} will contact ${I} on ${E} (${y}).`);const B=`Hello V. Ramana, I have requested a callback on ${E} (${y}) regarding ${k} in ${F}.
Name: ${I}
Phone: ${w}${x?`
Note: `+x:""}`;window.open(`https://wa.me/${S.whatsapp.replace("+","")}?text=${encodeURIComponent(B)}`,"_blank")})}else if(o.activeModal==="edit-property"&&e){let w=function(){if(A){if(I&&(I.textContent=String(g.length)),g.length===0){A.innerHTML='<div style="font-size: 0.82rem; color: var(--text-muted); font-style: italic;">No photos yet. Please add at least one photo below.</div>';return}A.innerHTML=g.map((E,y)=>`
      ...
      `).join(""),A.querySelectorAll("[data-img-idx]").forEach(E=>{E.addEventListener("click",y=>{y.preventDefault(),y.stopPropagation();const x=Number(E.dataset.imgIdx);g.splice(x,1),w()})})}};var r=w;if(!i){o.openModal("property-details",e),$("🔒 Access restricted: Only administrators can edit properties.");return}let g=Array.isArray(e.images)&&e.images.length>0?[...e.images]:[];a.innerHTML=`
              ...
              <div class="modal-grid-2col">
                <div class="input-field-group">
                  ...
                  <select id="edit-p-bhk" style="width: 100%; padding: 0.75rem; border-radius: 10px; border: 1px solid var(--border-color); background: var(--bg-input); color: var(--text-primary); font-size: 0.92rem; box-sizing: border-box;">
                    ...
                    <option value="2 BHK" ${e.bhk==="2 BHK"||(u=e.bhk)!=null&&u.includes("2")?"selected":""}>2 BHK</option>
    ...
    `;const S=()=>{o.openModal("property-details",e)};(P=document.getElementById("btn-back-from-edit"))==null||P.addEventListener("click",S),(C=document.getElementById("btn-close-edit"))==null||C.addEventListener("click",S),(N=document.getElementById("btn-cancel-edit-form"))==null||N.addEventListener("click",S);const A=document.getElementById("edit-photos-preview"),I=document.getElementById("edit-photos-count");w();const k=document.getElementById("edit-p-file"),F=document.getElementById("edit-p-upload-status");k==null||k.addEventListener("change",E=>{const y=Array.from(E.target.files||[]);if(!y.length)return;F&&(F.style.display="block",F.innerHTML=`<i class="fa-solid fa-spinner fa-spin"></i> Reading ${y.length} photo(s)...`);let x=0;y.forEach(B=>{const z=new FileReader;z.onload=L=>{g.push(L.target.result),x++,x===y.length&&(F&&(F.innerHTML=`✅ Added ${y.length} new photo(s)!`,setTimeout(()=>{F&&(F.style.display="none")},2e3)),w())},z.readAsDataURL(B)})}),(q=document.getElementById("form-edit-property"))==null||q.addEventListener("submit",async E=>{E.preventDefault();const y=document.getElementById("btn-save-edit-form");y&&(y.disabled=!0,y.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Saving changes...');const x=document.getElementById("edit-p-bhk").value,B=x==="1 BHK"?"1bhk":x==="2 BHK"?"2bhk":x==="3 BHK"?"3bhk":"4bhk",z=Array.from(document.querySelectorAll('input[name="edit-amenity"]:checked')).map(h=>h.value),L={title:document.getElementById("edit-p-title").value.trim(),locality:document.getElementById("edit-p-locality").value.trim(),address:document.getElementById("edit-p-address").value.trim(),price:Number(document.getElementById("edit-p-price").value),deposit:Number(document.getElementById("edit-p-deposit").value),bhk:x,bhkType:B,sqft:Number(document.getElementById("edit-p-sqft").value),furnishing:document.getElementById("edit-p-furnishing").value,floor:document.getElementById("edit-p-floor").value.trim(),facing:document.getElementById("edit-p-facing").value,bathrooms:Number(document.getElementById("edit-p-bathrooms").value),availableFrom:document.getElementById("edit-p-available").value.trim()||"Immediate",preferredTenants:document.getElementById("edit-p-tenants").value,amenities:z,images:g.length>0?g:e.images,description:document.getElementById("edit-p-desc").value.trim(),ownerName:document.getElementById("edit-p-owner-name").value.trim(),ownerPhone:document.getElementById("edit-p-owner-phone").value.trim()};await o.updateProperty(e.id,L),$(`✨ Property "${L.title}" updated successfully!`),o.openModal("property-details",{...e,...L})})}(j=document.getElementById("btn-close-modal"))==null||j.addEventListener("click",()=>{o.closeModal()}),(_=document.getElementById("modal-backdrop"))==null||_.addEventListener("click",g=>{g.target.id==="modal-backdrop"&&o.closeModal()})}async function re(a,e=1600,i=1600,r=.82){return typeof a=="string"&&a.startsWith("data:")?{dataUrl:a,fileName:"property-photo.jpg",mimeType:"image/jpeg",originalSize:a.length,compressedSize:a.length}:new Promise((l,s)=>{const t=a.name||"property-photo.jpg";if(!(a.type&&a.type.startsWith("image/")||/\.(jpe?g|png|webp|heic|heif|bmp|gif)$/i.test(t)||!a.type)&&a.type)return s(new Error("Selected file is not an image"));const c=URL.createObjectURL(a),m=new Image;m.onload=()=>{try{URL.revokeObjectURL(c);let{width:p,height:n}=m;(p>e||n>i)&&(p/n>e/i?(n=Math.round(n*e/p),p=e):(p=Math.round(p*i/n),n=i)),p=Math.max(1,p),n=Math.max(1,n);const d=document.createElement("canvas");d.width=p,d.height=n;const u=d.getContext("2d");if(!u)throw new Error("Canvas 2D context unavailable");u.fillStyle="#ffffff",u.fillRect(0,0,p,n),u.drawImage(m,0,0,p,n);const b=d.toDataURL("image/jpeg",r);l({dataUrl:b,fileName:t.replace(/\.[^/.]+$/,"")+".jpg",mimeType:"image/jpeg",width:p,height:n,originalSize:a.size,compressedSize:Math.round(b.length*3/4)})}catch(p){console.warn("Canvas compression fallback due to:",p),G(a,t,l,s)}},m.onerror=()=>{URL.revokeObjectURL(c),G(a,t,l,s)},m.src=c})}function G(a,e,i,r){const l=new FileReader;l.onload=s=>{i({dataUrl:s.target.result,fileName:e.replace(/\.[^/.]+$/,"")+".jpg",mimeType:a.type||"image/jpeg",width:800,height:600,originalSize:a.size,compressedSize:a.size})},l.onerror=()=>r(new Error("Failed to read image file on this device")),l.readAsDataURL(a)}async function X(a,e=null){const i=Array.from(a),r=[];for(let l=0;l<i.length;l++){const s=i[l];e&&e(l+1,i.length);try{const t=await re(s);r.push(t)}catch(t){console.warn(`Could not compress file ${s.name||l}:`,t)}}return r}function se(){var c,m,p;if(o.activeModal!=="list-property")return;const a=document.getElementById("modal-root");if(!a)return;a.innerHTML=`
    <div class="modal-overlay" id="modal-backdrop">
  ...
  `;const e=document.getElementById("lp-file-input"),i=document.getElementById("lp-upload-dropzone"),r=document.getElementById("lp-image-preview"),l=document.getElementById("lp-upload-status");let s=[];i==null||i.addEventListener("dragover",n=>{n.preventDefault(),i.style.borderColor="var(--accent-indigo)"}),i==null||i.addEventListener("dragleave",()=>{i.style.borderColor="var(--border-color)"}),i==null||i.addEventListener("drop",n=>{var d;n.preventDefault(),i.style.borderColor="var(--border-color)",(d=n.dataTransfer.files)!=null&&d.length&&t(Array.from(n.dataTransfer.files))}),e==null||e.addEventListener("change",n=>{var d;(d=n.target.files)!=null&&d.length&&t(Array.from(n.target.files))});async function t(n){if(!(!n||n.length===0)){l&&(l.style.display="block",l.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Optimizing mobile photos for fast upload...');try{const d=await X(n,(u,b)=>{l&&(l.innerHTML=`<i class="fa-solid fa-spinner fa-spin"></i> Optimizing photo ${u} of ${b}...`)});d.forEach(u=>{s.push(u)}),l&&(l.innerHTML=`✅ ${d.length} photo(s) optimized & ready!`,setTimeout(()=>{l&&(l.style.display="none")},2500))}catch(d){console.error("Error processing mobile images:",d),$("⚠️ Could not process image, please try another file."),l&&(l.style.display="none")}f()}}function f(){if(r){if(s.length===0){r.innerHTML="";return}r.innerHTML=s.map((n,d)=>{const u=typeof n=="string"?n:n.dataUrl,b=n.compressedSize?`${Math.round(n.compressedSize/1024)} KB`:"";return`
        <div style="position: relative; display: inline-block; margin: 4px;">
          <img src="${u}" style="width: 76px; height: 76px; border-radius: 10px; object-fit: cover; border: 2px solid var(--accent-indigo); display: block;" />
      ...
      `}).join(""),r.querySelectorAll(".btn-remove-lp-img").forEach(n=>{n.addEventListener("click",d=>{d.preventDefault(),d.stopPropagation();const u=Number(n.dataset.imgIdx);s.splice(u,1),f()})})}}(c=document.getElementById("form-list-property"))==null||c.addEventListener("submit",async n=>{var F,E;n.preventDefault();const d=document.getElementById("btn-submit-listing");d&&(d.disabled=!0,d.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Saving listing...');const u=document.getElementById("lp-title").value,b=document.getElementById("lp-locality").value,v=document.getElementById("lp-bhk").value,P=v==="1bhk"?"1 BHK":v==="2bhk"?"2 BHK":v==="3bhk"?"3 BHK":"4+ BHK",C=Number(document.getElementById("lp-price").value),N=Number(document.getElementById("lp-deposit").value),q=Number(document.getElementById("lp-sqft").value),j=document.getElementById("lp-furnishing").value,_=document.getElementById("lp-owner-name").value,g=document.getElementById("lp-owner-phone").value,S=((F=document.getElementById("lp-floor"))==null?void 0:F.value.trim())||"3rd Floor",A=((E=document.getElementById("lp-facing"))==null?void 0:E.value)||"East Facing",I=Array.from(document.querySelectorAll('input[name="lp-amenity"]:checked')).map(y=>y.value),w=`prop-custom-${Date.now()}`;let k=[];if(s.length>0)for(let y=0;y<s.length;y++){const x=s[y],B=typeof x=="string"?x:x.dataUrl,z=typeof x=="object"&&x.fileName?x.fileName:`listing-${y+1}.jpg`,L=typeof x=="object"&&x.mimeType?x.mimeType:"image/jpeg";d&&(d.innerHTML=`<i class="fa-solid fa-spinner fa-spin"></i> Uploading photo ${y+1} of ${s.length}...`);try{const h=await M.uploadImage(B,w,z,L);h&&h.url?k.push(h.url):k.push(B)}catch(h){console.warn("Image upload fallback:",h),k.push(B)}}else k=["https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?auto=format&fit=crop&w=1200&q=80"];d&&(d.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Finalizing listing...'),await o.addProperty({id:w,title:u,locality:b,address:`${b}, Bengaluru`,price:C,deposit:N,bhk:P,bhkType:v,type:"Apartment",furnishing:j,sqft:q,bathrooms:2,floor:S,facing:A,availableFrom:"Immediate",preferredTenants:"Any",images:k,amenities:I.length>0?I:["Power Backup","Lift","Car Parking","24/7 Security"],description:`Newly listed ${P} apartment in prime ${b}. Directly posted by property owner.`,ownerName:_,ownerPhone:g,ownerType:"Direct Owner"}),o.closeModal(),$(`✨ Property "${u}" in ${b} listed successfully!`)}),(m=document.getElementById("btn-close-modal"))==null||m.addEventListener("click",()=>{o.closeModal()}),(p=document.getElementById("modal-backdrop"))==null||p.addEventListener("click",n=>{n.target.id==="modal-backdrop"&&o.closeModal()})}function V(a){return typeof a!="string"?a:a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function le(){var r,l,s,t,f,c;if(o.activeModal!=="auth-signin")return;const a=document.getElementById("modal-root");if(!a)return;const e=o.userAuthMode==="signin";a.innerHTML=`
    <div class="modal-overlay" id="modal-backdrop">
  ...
  `,(r=document.getElementById("btn-switch-signin"))==null||r.addEventListener("click",()=>{o.userAuthMode="signin",o.notify()}),(l=document.getElementById("btn-switch-register"))==null||l.addEventListener("click",()=>{o.userAuthMode="register",o.notify()}),(s=document.getElementById("form-user-login"))==null||s.addEventListener("submit",async m=>{var P,C;m.preventDefault();const p=((P=document.getElementById("user-login-email"))==null?void 0:P.value)||"",n=((C=document.getElementById("user-login-pass"))==null?void 0:C.value)||"",d=V(p.trim()),u=n.trim(),b=m.target.querySelector('button[type="submit"]');b&&(b.disabled=!0,b.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Authenticating...');const v=await o.loginUser(d,u);v.success?v.isAdmin?$("⚡ Logged in as Proprietor Admin!"):$(`🎉 Welcome back, ${v.user.name}!`):(b&&(b.disabled=!1,b.innerHTML='<i class="fa-solid fa-right-to-bracket"></i> Secure Sign In'),$(`❌ ${v.message}`))}),(t=document.getElementById("form-user-register"))==null||t.addEventListener("submit",async m=>{var j,_,g,S;m.preventDefault();const p=((j=document.getElementById("user-reg-name"))==null?void 0:j.value)||"",n=((_=document.getElementById("user-reg-email"))==null?void 0:_.value)||"",d=((g=document.getElementById("user-reg-pass"))==null?void 0:g.value)||"",u=((S=document.getElementById("user-reg-confirm"))==null?void 0:S.value)||"",b=V(p.trim()),v=V(n.trim()),P=d.trim(),C=u.trim();if(P!==C){$("⚠️ Passwords do not match! Please check again.");return}const N=m.target.querySelector('button[type="submit"]');N&&(N.disabled=!0,N.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Creating Account in DB...');const q=await o.registerUser({name:b,email:v,password:P});q.success?$(`🎉 Account created! Welcome, ${q.user.name}.`):(N&&(N.disabled=!1,N.innerHTML='<i class="fa-solid fa-user-plus"></i> Create My Account'),$(`❌ ${q.message}`))});const i=(m,p,n)=>{const d=document.getElementById(m),u=document.getElementById(p),b=document.getElementById(n);!d||!u||d.addEventListener("click",v=>{v.preventDefault(),v.stopPropagation();const P=u.type==="password";u.type=P?"text":"password",b&&(P?(b.className="fa-solid fa-eye-slash",d.style.color="var(--accent-emerald)",d.setAttribute("title","Hide password")):(b.className="fa-solid fa-eye",d.style.color="var(--text-muted)",d.setAttribute("title","Show password"))),u.focus()})};i("btn-toggle-login-pass","user-login-pass","icon-login-pass-eye"),i("btn-toggle-reg-pass","user-reg-pass","icon-reg-pass-eye"),i("btn-toggle-confirm-pass","user-reg-confirm","icon-confirm-pass-eye"),(f=document.getElementById("btn-close-modal"))==null||f.addEventListener("click",()=>{o.closeModal()}),(c=document.getElementById("modal-backdrop"))==null||c.addEventListener("click",m=>{m.target.id==="modal-backdrop"&&o.closeModal()})}function ne(){if(o.activeModal!=="admin-portal")return;const a=document.getElementById("modal-root");if(!a)return;(typeof o.isAdmin=="function"?o.isAdmin():!1)?ce(a):de(a)}function de(a){var f,c,m,p;a.innerHTML=`
  ...
  `;const e=document.getElementById("admin-pass-input"),i=document.getElementById("btn-toggle-admin-pass"),r=document.getElementById("btn-toggle-admin-pass-top"),l=document.getElementById("check-show-admin-pass");function s(n){if(!e)return;e.type=n?"text":"password",l&&(l.checked=n);const d=document.getElementById("icon-admin-pass-eye"),u=document.getElementById("text-admin-pass-toggle");d&&(d.className=n?"fa-solid fa-eye-slash":"fa-solid fa-eye"),u&&(u.textContent=n?"Hide":"Show");const b=document.getElementById("icon-admin-pass-eye-top"),v=document.getElementById("text-admin-pass-top");b&&(b.className=n?"fa-solid fa-eye-slash":"fa-solid fa-eye"),v&&(v.textContent=n?"Hide Password":"Show Password"),e.focus()}i==null||i.addEventListener("click",n=>{n.preventDefault();const d=(e==null?void 0:e.type)==="password";s(d)}),r==null||r.addEventListener("click",n=>{n.preventDefault();const d=(e==null?void 0:e.type)==="password";s(d)}),l==null||l.addEventListener("change",n=>{s(n.target.checked)}),(f=document.getElementById("admin-login-form"))==null||f.addEventListener("submit",async n=>{n.preventDefault();const d=document.getElementById("admin-email-input").value.trim(),u=document.getElementById("admin-pass-input").value.trim(),b=document.getElementById("btn-submit-admin-login"),v=document.getElementById("admin-auth-error-box"),P=document.getElementById("admin-auth-error-msg");if(v&&(v.style.display="none"),u.toLowerCase()==="ramana rentals"){v&&P&&(P.innerText="Access Denied: Legacy password is no longer valid. Please use ramana@123",v.style.display="block"),$("❌ Access Denied: Legacy password is no longer valid!");return}b&&(b.disabled=!0,b.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Verifying credentials...'),await o.adminLogin(d,u)?(o.pendingAdminTab&&(o.setAdminTab(o.pendingAdminTab),o.pendingAdminTab=null),$("⚡ Administrator Access Granted! Welcome V. RAMANA.")):(b&&(b.disabled=!1,b.innerHTML='<i class="fa-solid fa-lock-open"></i> Authenticate & Unlock Portal'),v&&P&&(P.innerText="Access Denied: Invalid administrator credentials.",v.style.display="block"),$("❌ Access Denied: Invalid credentials!"))});const t=()=>{o.closeModal(),(window.location.pathname.toLowerCase().replace(/\/$/,"")==="/admin"||window.location.hash.toLowerCase()==="#admin"||window.location.hash.toLowerCase()==="#/admin")&&history.replaceState(null,"","/")};(c=document.getElementById("btn-close-admin-modal"))==null||c.addEventListener("click",t),(m=document.getElementById("btn-return-home"))==null||m.addEventListener("click",n=>{n.preventDefault(),t()}),(p=document.getElementById("admin-modal-backdrop"))==null||p.addEventListener("click",n=>{n.target.id==="admin-modal-backdrop"&&t()})}function ce(a){var m,p;const e=o.allProperties,i=o.leads,r=o.contactInfo,l=e.reduce((n,d)=>n+(d.price||0),0),s=e.filter(n=>n.isVerified).length,t=o.adminTab||"dashboard";a.innerHTML=`
        ...
        <div class="admin-header-left">
          ...
          <div style="min-width: 0; flex: 1;">
            ...
            <p class="admin-subtitle-info">
              Logged in as <strong>${r.proprietor} (${r.role})</strong> | Phone: ${r.phone}
            </p>
          </div>
        </div>
  ...
  `,window.scrollTo(0,0),document.documentElement.scrollTop=0,document.body.scrollTop=0;const f=a.querySelector("#admin-portal-shell");f&&f.addEventListener("scroll",()=>{f.scrollTop!==0&&(f.scrollTop=0),f.scrollLeft!==0&&(f.scrollLeft=0)}),a.querySelectorAll("[data-admin-tab]").forEach(n=>{n.addEventListener("click",()=>{o.setAdminTab(n.dataset.adminTab);const d=a.querySelector(".admin-body-container");d&&(d.scrollTop=0)})});const c=(n=!1)=>{n?(o.adminLogout(),$("Logged out of Admin Portal.")):o.closeModal(),(window.location.pathname.toLowerCase().replace(/\/$/,"")==="/admin"||window.location.hash.toLowerCase()==="#admin")&&history.replaceState(null,"","/")};(m=document.getElementById("btn-admin-logout"))==null||m.addEventListener("click",()=>c(!0)),(p=document.getElementById("btn-close-admin-portal"))==null||p.addEventListener("click",()=>c(!1)),me(t,a)}function pe(a,e,i,r,l,s){return a==="dashboard"?`
      <!-- KPI Stats Grid -->
        ...
        <div style="display: flex; flex-direction: column; gap: 1rem;">
          <div class="admin-card-box">
            ...
            <div style="font-size: 0.9rem; line-height: 1.6;">
              <div><strong>Name:</strong> ${r.proprietor} (${r.role})</div>
              <div><strong>Phone:</strong> ${r.phone}</div>
              <div><strong>WhatsApp:</strong> ${r.whatsapp}</div>
              <div><strong>Email:</strong> ${r.email}</div>
              <div style="margin-top: 0.5rem; font-size: 0.8rem; color: var(--text-secondary);"><strong>Office:</strong> ${r.address}</div>
            </div>
        ...
        <form id="admin-settings-form" style="display: flex; flex-direction: column; gap: 1.25rem;">
          <div class="responsive-form-row">
            <div class="input-field-group">
              ...
              <input type="text" id="set-proprietor" value="${r.proprietor}" required />
            </div>

            <div class="input-field-group">
              <label>Role / Title</label>
              <input type="text" id="set-role" value="${r.role}" required />
            </div>
          </div>
          ...
          <div class="responsive-form-row">
            <div class="input-field-group">
              <label>Contact Phone Number</label>
              <input type="tel" id="set-phone" value="${r.phone}" required />
            </div>

            <div class="input-field-group">
              <label>WhatsApp Number</label>
              <input type="tel" id="set-whatsapp" value="${r.whatsapp}" required />
            </div>
          </div>

          <div class="input-field-group">
            <label>Business Email</label>
            <input type="email" id="set-email" value="${r.email}" required />
          </div>

          <div class="input-field-group">
            <label>Office Address</label>
            <textarea id="set-address" rows="2" style="width: 100%; padding: 0.75rem; border-radius: 10px; border: 1px solid var(--border-color); background: var(--bg-input); color: var(--text-primary);" required>${r.address}</textarea>
          </div>

          <div class="input-field-group">
            <label>Business Slogan</label>
            <input type="text" id="set-slogan" value="${r.slogan}" required />
          </div>
    ...
    `:""}function me(a,e){var r,l;if(e.querySelectorAll("[data-admin-tab-goto]").forEach(s=>{s.addEventListener("click",()=>{o.setAdminTab(s.dataset.adminTabGoto);const t=e.querySelector(".admin-body-container");t&&(t.scrollTop=0)})}),a==="properties"){const s=document.getElementById("admin-prop-search"),t=document.getElementById("admin-prop-floor-filter"),f=()=>{const c=(s==null?void 0:s.value.toLowerCase().trim())||"",m=(t==null?void 0:t.value)||"All";e.querySelectorAll("#admin-properties-table tbody tr").forEach(p=>{var v;const n=p.innerText.toLowerCase(),d=!c||n.includes(c),u=((v=p.querySelector(".admin-floor-select"))==null?void 0:v.value)||"",b=m==="All"||u.toLowerCase()===m.toLowerCase();p.style.display=d&&b?"":"none"}),e.querySelectorAll("[data-mobile-prop-card]").forEach(p=>{var v;const n=p.innerText.toLowerCase(),d=!c||n.includes(c),u=((v=p.querySelector(".admin-floor-select"))==null?void 0:v.value)||"",b=m==="All"||u.toLowerCase()===m.toLowerCase();p.style.display=d&&b?"":"none"})};s==null||s.addEventListener("input",f),t==null||t.addEventListener("change",f),e.querySelectorAll(".admin-floor-select").forEach(c=>{c.addEventListener("change",async()=>{const m=c.dataset.propId,p=c.value;await o.updatePropertyFloor(m,p),$(`🏢 Floor updated to "${p}"!`)})}),e.querySelectorAll(".flag-toggle-btn").forEach(c=>{c.addEventListener("click",()=>{const m=c.dataset.flagProp,p=c.dataset.flagName;o.togglePropertyFlag(m,p),$(`Updated ${p} flag!`)})}),e.querySelectorAll("[data-del-prop]").forEach(c=>{c.addEventListener("click",async()=>{const m=c.dataset.delProp;if(confirm("Are you sure you want to delete this property listing?")){const p=await o.deleteProperty(m);p&&p.success?$("🗑️ Property deleted successfully from Cloud DB."):$(`❌ Delete failed: ${(p==null?void 0:p.error)||"Unauthorized"}. Please re-login as Admin.`)}})}),e.querySelectorAll("[data-edit-prop]").forEach(c=>{c.addEventListener("click",()=>{const m=c.dataset.editProp,p=o.allProperties.find(n=>String(n.id)===String(m));p&&o.openModal("edit-property",p)})})}if(a==="add-property"){let n=function(){if(f){if(m.length===0){f.innerHTML="";return}f.innerHTML=m.map((d,u)=>{const b=typeof d=="string"?d:d.dataUrl,v=d.compressedSize?`${Math.round(d.compressedSize/1024)} KB`:"";return`
          <div style="position: relative; display: inline-block; margin: 4px;">
            ...
            <button type="button" class="btn-remove-img" data-img-idx="${u}" style="position: absolute; top: -8px; right: -8px; width: 28px; height: 28px; border-radius: 50%; background: #ef4444; color: #fff; border: 2px solid #fff; font-size: 0.8rem; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 6px rgba(0,0,0,0.5); z-index: 10; touch-action: manipulation;" title="Remove Image">
              <i class="fa-solid fa-xmark"></i>
            </button>
          </div>
        `}).join(""),f.querySelectorAll("[data-img-idx]").forEach(d=>{d.addEventListener("click",u=>{u.preventDefault(),u.stopPropagation();const b=Number(d.dataset.imgIdx);m.splice(b,1),n()})})}};var i=n;const s=document.getElementById("admin-p-file"),t=document.getElementById("admin-p-upload-area"),f=document.getElementById("admin-p-image-preview"),c=document.getElementById("admin-p-upload-status");let m=[];t==null||t.addEventListener("dragover",d=>{d.preventDefault(),t.style.borderColor="var(--accent-emerald)",t.style.background="rgba(16, 185, 129, 0.15)"}),t==null||t.addEventListener("dragleave",()=>{t.style.borderColor="var(--accent-emerald)",t.style.background="rgba(16, 185, 129, 0.05)"}),t==null||t.addEventListener("drop",d=>{var u;d.preventDefault(),t.style.background="rgba(16, 185, 129, 0.05)",(u=d.dataTransfer.files)!=null&&u.length&&p(Array.from(d.dataTransfer.files))}),s==null||s.addEventListener("change",d=>{var u;(u=d.target.files)!=null&&u.length&&p(Array.from(d.target.files))});async function p(d){if(!(!d||d.length===0)){c&&(c.style.display="block",c.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Optimizing mobile photos for fast upload...');try{const u=await X(d,(b,v)=>{c&&(c.innerHTML=`<i class="fa-solid fa-spinner fa-spin"></i> Optimizing photo ${b} of ${v}...`)});u.forEach(b=>{m.push(b)}),c&&(c.innerHTML=`✅ ${u.length} photo(s) optimized & ready!`,setTimeout(()=>{c&&(c.style.display="none")},2500))}catch(u){console.error("Error processing mobile images:",u),$("⚠️ Could not process image, please try another file."),c&&(c.style.display="none")}n()}}(r=document.getElementById("admin-add-prop-form"))==null||r.addEventListener("submit",async d=>{var x,B;d.preventDefault();const u=d.target.querySelector('button[type="submit"]');u&&(u.disabled=!0,u.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Saving property...');const b=document.getElementById("admin-p-title").value,v=document.getElementById("admin-p-address").value,P=document.getElementById("admin-p-locality").value,C=document.getElementById("admin-p-bhk").value,N=C==="1bhk"?"1 BHK":C==="2bhk"?"2 BHK":C==="3bhk"?"3 BHK":"4+ BHK",q=Number(document.getElementById("admin-p-price").value),j=Number(document.getElementById("admin-p-deposit").value),_=Number(document.getElementById("admin-p-sqft").value),g=document.getElementById("admin-p-furnishing").value,S=document.getElementById("admin-p-owner-name").value,A=document.getElementById("admin-p-owner-phone").value,I=document.getElementById("admin-p-desc").value,w=((x=document.getElementById("admin-p-floor"))==null?void 0:x.value.trim())||"3rd Floor",k=((B=document.getElementById("admin-p-facing"))==null?void 0:B.value)||"East Facing",F=Array.from(document.querySelectorAll('input[name="admin-amenity"]:checked')).map(z=>z.value),E=`prop-custom-${Date.now()}`;let y=[];if(m.length>0)for(let z=0;z<m.length;z++){const L=m[z],h=typeof L=="string"?L:L.dataUrl,O=typeof L=="object"&&L.fileName?L.fileName:`admin-photo-${z+1}.jpg`,R=typeof L=="object"&&L.mimeType?L.mimeType:"image/jpeg";u&&(u.innerHTML=`<i class="fa-solid fa-spinner fa-spin"></i> Uploading photo ${z+1} of ${m.length}...`);try{const D=await M.uploadImage(h,E,O,R);D&&D.url?y.push(D.url):y.push(h)}catch(D){console.warn("Image upload fallback to dataUrl:",D),y.push(h)}}else y=["https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?auto=format&fit=crop&w=1200&q=80"];u&&(u.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Storing property...'),await o.addProperty({id:E,title:b,locality:P,address:v,price:q,deposit:j,bhk:N,bhkType:C,type:"Apartment",furnishing:g,sqft:_,bathrooms:2,floor:w,facing:k,availableFrom:"Immediate",preferredTenants:"Any",images:y,amenities:F.length>0?F:["Power Backup","Lift","Car Parking","24/7 Security"],description:I,ownerName:S,ownerPhone:A,ownerType:"Direct Owner"}),$(`✨ Property "${b}" published successfully!`),o.setAdminTab("properties")})}a==="leads"&&(e.querySelectorAll(".lead-status-select").forEach(s=>{s.addEventListener("change",t=>{const f=s.dataset.leadId;o.updateLeadStatus(f,t.target.value),$(`Updated lead status to ${t.target.value}`)})}),e.querySelectorAll("[data-del-lead]").forEach(s=>{s.addEventListener("click",()=>{const t=s.dataset.delLead;confirm("Delete this tenant lead?")&&(o.deleteLead(t),$("Lead deleted."))})})),a==="settings"&&((l=document.getElementById("admin-settings-form"))==null||l.addEventListener("submit",s=>{s.preventDefault();const t=document.getElementById("set-proprietor").value,f=document.getElementById("set-role").value,c=document.getElementById("set-phone").value,m=document.getElementById("set-whatsapp").value,p=document.getElementById("set-email").value,n=document.getElementById("set-address").value,d=document.getElementById("set-slogan").value,u=c.replace(/\D/g,""),b=u.length===10?`+91${u}`:`+${u}`;o.updateContactInfo({proprietor:t,role:f,phone:c,phoneRaw:b,whatsapp:m,email:p,address:n,slogan:d}),$("💾 Proprietor settings updated across the site!")}))}function ue(){var i;const a=document.getElementById("footer-root");if(!a)return;const e=o.contactInfo;a.innerHTML=`
  ...
  `,(i=document.getElementById("footer-btn-contact-modal"))==null||i.addEventListener("click",()=>{o.openModal("contact-us")})}let W=!1;function J(){const a=document.activeElement,e=a&&(a.id==="hero-place-search-input"||a.id==="filter-search-input");if(ae(),(!e||!W)&&(ee(),te(),ie(),ue()),oe(),se(),le(),ne(),document.body.classList.toggle("modal-open",!!o.activeModal),a&&document.body.contains(a))try{a.focus()}catch{}W=!0}function K(){const a=window.location.pathname.toLowerCase().replace(/\/$/,""),e=window.location.hash.toLowerCase();(a==="/admin"||e==="#admin")&&(typeof o.isAdmin=="function"?o.isAdmin():o.isAdminLoggedIn)&&o.activeModal!=="admin-portal"&&o.openModal("admin-portal")}function Y(){var a;document.documentElement.setAttribute("data-theme",o.theme),K(),window.addEventListener("popstate",K),window.addEventListener("hashchange",K),J(),(a=document.getElementById("floating-btn-book-call"))==null||a.addEventListener("click",()=>{o.openModal("book-call")}),o.subscribe(()=>{J()})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Y):Y();
